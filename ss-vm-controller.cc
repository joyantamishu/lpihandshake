...
int BaseTopology::getCustomizedRandomClientNode(uint32_t &application_id)
{
	uint32_t total_hosts = hosts.GetN();

	NS_LOG_UNCOND("@@@@@@@@@@@getCustomizedRandomClientNode @@@@@@@@@@@@@@@@ "<<BaseTopology::finished_application_list.size());
	if(BaseTopology::finished_application_list.size() > 0)
	{
		int val = BaseTopology::finished_application_list.front();

		BaseTopology::finished_application_list.pop_front();

		application_id = val;


		NS_LOG_UNCOND("The app id "<<application_id);
...
//	for(application_index=0;application_index<simulationRunProperties::total_applications;application_index++)
//	{
...
int BaseTopology::getCustomizedRandomClientNodeDummy(uint32_t &application_id)
{
	uint32_t total_hosts = hosts.GetN();
	...
	uint32_t application_index = 0;
	for(application_index=simulationRunProperties::total_applications;application_index < simulationRunProperties::total_applications+DEFAULT_NUMBER_OF_DUMMY_APPLICATIONS;application_index++)
	{
		sum_application_probability += BaseTopology::application_probability[application_index];
