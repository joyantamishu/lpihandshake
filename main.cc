...
//				"\nStatistics: JB: The avg delay of flows " << sum_avg_delay/(double)flow_count<<" Flow Count "<<flow_count<<" sum_avg_delay "<<sum_avg_delay);
	...
	{	fp_stat = fopen ("Statistics.csv","w");
		fprintf(fp_stat,"Total number of Flows Generated, Total Packets Destination,Currently No of pkts in system, Avg Delay(ms), Avg Delay(s), Average_delay_no_burst(sec),Avg Delay in bursts(sec),Tail Latency(microsec),Total Rollback Pkts,#Copy Created, #Copy Deleted, Consistency Pkts, Non-Consistency Pkts,Copy Creation Pkts \n");
	}
