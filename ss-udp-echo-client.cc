	...
	if( simulationRunProperties::enableOptimizer)
	{
		...
		if(BaseTopology::Incrcounter_%5==0)
		{
			...
			while(BaseTopology::res[i].src != 99999)// && BaseTopology::res!=NULL)
			{
				...
				uint32_t min_node;
				//if we need to create a copy of the chunk then only increase the chunk number and else not
				if (BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).copy_vs_move==1) //which means copy //Added by Madhurima on Apr 17
				{
					...
					BaseTopology::copy_moved++;
					p='m';
					for (uint32_t t=start;t<(start+SSD_PER_RACK);t++)
					{
						if(Ipv4GlobalRouting::host_utilization[t]<minimim_utilization && !BaseTopology::chunk_copy_node_tracker[BaseTopology::res[i].chunk_number][t] )//&& BaseTopology::host_copy[t]==0) //make sure no duplicate assignment while assigning
						{
							minimim_utilization=Ipv4GlobalRouting::host_utilization[t];
							//minimim_utilization=BaseTopology::host_utilization_outgoing[t];
							min_node=t;
				...
				NS_ASSERT_MSG(min_node%(SSD_PER_RACK + 1)!=0,"trying to create copy at the application node beware of it");
...
//						}
				...
				if (BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).copy_vs_move==0) //Added by Madhurima on Apr 17
				{
				...
				uint32_t location;
				//checking whether the chunk already exists and then getting the chunk location
				for(uint32_t chunk_index = 0 ;chunk_index < BaseTopology::p[pod].nodes[node].total_chunks;chunk_index++)
				{
				...
				if (BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).copy_vs_move==0 && BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).number_of_copy > 0)
				{
...
double ssUdpEchoClient::hs_sum(uint64_t n, uint64_t alpha){
	double summ=0.0;
	double j,val;
		...
		for(uint32_t i=1 ; i<= ns3::BaseTopology::chunk_assignment_to_applications[application_index][0];i++)
		{
			...
			for(uint32_t chunk_index = 0 ;chunk_index < BaseTopology::p[pod].nodes[node].total_chunks;chunk_index++)
			{
				if(BaseTopology::p[pod].nodes[node].data[chunk_index].chunk_number == chunk_value)
				{
					...
					{
						BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum = 0;//ceil(BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum );
					}
					break;
				}
			}
		...
		if(simulationRunProperties::enableOptimizer)
		{
			...
			if(BaseTopology::counter_==0 )
			{
				...
				while(BaseTopology::res[i].src != 99999 && BaseTopology::res!=NULL)
				{
					...
					{
						//getting the ssd from where we have to remove the chunk
						uint32_t chunk_location = FindChunkAssignedHost(BaseTopology::res[i].chunk_number, BaseTopology::res[i].src);
						//uint32_t round_robin_counter = BaseTopology::host_assignment_round_robin_counter[BaseTopology::res[i].dest] % SSD_PER_RACK;
						BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).number_of_copy --;
						BaseTopology::chunk_copy_node_tracker[BaseTopology::res[i].chunk_number][chunk_location] = false;

						//getting the ssd to where we have to move the traffic pertaining to the chunk from now on
						chunk_location = FindChunkAssignedHost(BaseTopology::res[i].chunk_number, BaseTopology::res[i].dest);
						BaseTopology::chunkTracker.at(BaseTopology::res[i].chunk_number).logical_node_id = chunk_location;//(SSD_PER_RACK + 1) * BaseTopology::res[i].dest + 1;
