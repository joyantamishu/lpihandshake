...
ApplicationContainer ssUdpEchoClientHelper::Install(Ptr<Node> node, bool single_destination_flow, uint32_t source_node, uint32_t app_id, uint32_t dest_node, uint32_t no_of_packets, bool read_flow, bool non_consistent_read_flow, uint32_t read_app_id, uint32_t trace_id) const {
	return ApplicationContainer(InstallPriv(node,single_destination_flow, source_node, app_id, dest_node, no_of_packets, read_flow, non_consistent_read_flow, read_app_id, trace_id));
}
...
Ptr<Application> ssUdpEchoClientHelper::InstallPriv(Ptr<Node> node, bool single_destination_flow, uint32_t source_node, uint32_t app_id, uint32_t dest_node, uint32_t no_of_packets, bool read_flow, bool non_consistent_read_flow, uint32_t read_app_id, uint32_t trace_id) const {
	Ptr<ssUdpEchoClient> app = m_factory.Create<ssUdpEchoClient>();

	NS_LOG_UNCOND("The source node is "<<source_node<<" read_app_id "<<read_app_id<<" trace_id "<<trace_id);
...
void ssUdpEchoClient::ScheduleTransmit(Time dt) {
	...
	if (!consistency_flow)
	{
		double val = m_randomVariableInterPacketInterval->GetValue();
		m_packetInterval = Time::FromDouble(val, Time::S);
...
void ssUdpEchoClient::CreateandRemoveIndependentReadFlows(uint32_t distinct_hosts, double finish_time, double cumulative_sum, uint32_t no_of_entries, int create)
{
	...
	for(uint32_t i=0;i<distinct_items;i++)
	{
		...
		for (uint32_t chunk_no = 0; chunk_no < no_of_entries; chunk_no++)
		{
			uint32_t chunk_location = getChunkLocation(entries[chunk_no].chunk_id, &version, &socket_index);

			if (chunk_location == ssd_host)
			{
				count_chunk++;
				double utilization = (entries[chunk_no].bandwidth/(double)MegabyteToByte) * 8.0;
				utilization = utilization /cumulative_sum;

				utilization = utilization * create;

				BaseTopology::p[pod].nodes[node].utilization_out += utilization;

				BaseTopology::host_utilization_outgoing[ssd_host] += utilization;
				...
				for(uint32_t chunk_index = 0 ;chunk_index < BaseTopology::p[pod].nodes[node].total_chunks;chunk_index++)
				{
					...
					if(BaseTopology::p[pod].nodes[node].data[chunk_index].chunk_number == entries[chunk_no].chunk_id)
					{
						...
						BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum_out += utilization;
		...
		if (create == 1 && count_chunk >= 1) BaseTopology::InjectANewRandomFlowCopyCreation(ssd_host, destination, 0, true, count_chunk, true, finish_time, working_app,trace_id);
	}
...
void ssUdpEchoClient::StartApplication() {
	...
	else
	{
		if(read_flow)
		{
			...
			flowentries = new singledatasetentry[ssUdpEchoClient::m_flowRequiredBW];
			...
			sprintf(assigned_sub_trace_file, "%s_%d_%s_%d.csv", "application",read_flow_application_index,"flow",trace_index);

			//NS_LOG_UNCOND(assigned_sub_trace_file);
			...
			while (fgets(str, 1000, fp) != NULL)
			{
				...
				bool is_read = true;
				if (io_type == 'W') is_read = false;

				uint32_t chunk_location = getChunkLocation(chunk_no, &version, &socket_index);

				if(chunk_location == ssd_host && is_read)
				{
					flowentries[count] = singledatasetentry(timestamp,is_read, chunk_no, size);
			...
			//NS_LOG_UNCOND("Here I am Leaving");
	...
	entries = new singledatasetentry[ENTRIES_PER_FLOW];
	...
	sprintf(assigned_sub_trace_file, "%s_%d_%s_%d.csv", "application",ssUdpEchoClient::working_app,"flow",ssUdpEchoClient::trace_id);
	...
	while (fgets(str, 1000, fp) != NULL)
	{
		...
		cumulative_sum += timestamp;
		bool is_read = true;

		if (io_type == 'W') is_read = false;


		entries[count] = singledatasetentry(timestamp,is_read, chunk_no, size);
	...
	timestamp_duration = cumulative_sum;

	for(uint32_t i=1 ; i<=count;i++)
	{
		//NS_LOG_UNCOND(getChunkLocation(entries[i-1].chunk_id, &version, &socket_index));
		uint32_t j;
		for (j=1; j<i; j++)
		{
			if (getChunkLocation(entries[i-1].chunk_id, &version, &socket_index) == getChunkLocation(entries[j-1].chunk_id, &version, &socket_index)) break;
		}
		if (i == j)
		{
			socket_mapping.push_back(MappingSocket(distinct_items, getChunkLocation(entries[i-1].chunk_id, &version, &socket_index)));
			distinct_items++;
		}

	}

	NS_LOG_UNCOND("Distinct Items "<<distinct_items);
	...
	for(uint32_t i=0 ; i<count;i++)
	{
		...
		{
			write_opt ++;
			double utilization = (entries[i].bandwidth/(double)MegabyteToByte) * 8.0;
			utilization = utilization /cumulative_sum;
			uint32_t chunk_location = getChunkLocation(entries[i].chunk_id, &version, &socket_index);
			Ipv4GlobalRouting::host_utilization[chunk_location] += utilization;
			...
			BaseTopology::p[pod].nodes[node].utilization += utilization;
			...
			for(uint32_t chunk_index = 0 ;chunk_index < BaseTopology::p[pod].nodes[node].total_chunks;chunk_index++)
			{
				...
				if(BaseTopology::p[pod].nodes[node].data[chunk_index].chunk_number == entries[i].chunk_id)
				{
					BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum += utilization;
					BaseTopology::p[pod].nodes[node].data[chunk_index].processed=0;
					break;
				}
			}
	...
	if(write_opt > 0 )
	{
		flowentries = new singledatasetentry[write_opt];



		for (uint32_t wc = 0; wc < count; wc++)
		{
			if(!entries[wc].is_read)
			{
				flowentries[w_index].chunk_id =  entries[wc].chunk_id;
				flowentries[w_index].bandwidth =  entries[wc].bandwidth;
				flowentries[w_index].is_read =  entries[wc].is_read;
				flowentries[w_index].timestamp =  entries[wc].timestamp;
				w_index++;
	...
	else no_packet_flow = true;

	active_entries = (int) w_index;

	CreateandRemoveIndependentReadFlows(distinct_items, m_finish_time, timestamp_duration, entry_count);
	...
	//NS_LOG_UNCOND("ssUdpEchoClient::working_app "<<ssUdpEchoClient::working_app<<" ssUdpEchoClient::trace_id "<<ssUdpEchoClient::trace_id);


	for(uint32_t i=0 ; i< count;i++)
	{
		//NS_LOG_UNCOND(entries[i].chunk_id);
		uint32_t chunk_location = local_chunkTracker.at(entries[i].chunk_id).node_id;

		for (std::vector<MappingSocket>::iterator it = socket_mapping.begin() ; it != socket_mapping.end(); ++it)
		{
			if(it->location == chunk_location) local_chunkTracker.at(entries[i].chunk_id).socket_index = it->socket_id;
		}
	}
...
void ssUdpEchoClient::StopApplication(void) {
	...
	if (!m_lastPacket && !consistency_flow && !read_flow) {
		...
		for(uint32_t i=0 ; i<(uint32_t)active_entries;i++)
		{
			//NS_LOG_UNCOND("Here We go");

			uint32_t chunk_value = flowentries[i].chunk_id;
			...
			utilization = utilization / timestamp_duration;
			...
			for(uint32_t chunk_index = 0 ;chunk_index < BaseTopology::p[pod].nodes[node].total_chunks;chunk_index++)
			{
				...
				{
					if(BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum>utilization)
					{BaseTopology::p[pod].nodes[node].data[chunk_index].intensity_sum -= utilization;
		...
		CreateandRemoveIndependentReadFlows(distinct_items, m_finish_time, timestamp_duration, entry_count, -1);
	...
	if(!this->consistency_flow && !read_flow)
	{
		...
		if (active_entries >0 )delete[] flowentries;

	}

	//NS_LOG_UNCOND("Here I am 7");
...
void ssUdpEchoClient::Send(void) {
	...
	//NS_LOG_UNCOND("Entered here send");

	if(no_packet_flow || local_count >= (active_entries -1))
	{
...
//	}
	...
	if(!consistency_flow)
	{
		...
		{
			chunk_value = flowentries[local_count].chunk_id;
	...
	if((!is_write || num_of_packets_to_send == 1) && !consistency_flow)
	{
		...
		uint32_t num_packets = (uint32_t)((double)flowentries[local_count].bandwidth / (double) simulationRunProperties::packetSize);

		m_txTrace(t_p);

		BaseTopology::total_packet_count += num_packets;
	...
	else
	{
		if(consistency_flow)
		{
			BaseTopology::total_packet_count += this->total_packets_to_send;
	...
	if (m_lastPacket) {
		...
		ScheduleTransmit(Time::FromDouble(flowentries[local_count -1].timestamp, Time::S));
	}
